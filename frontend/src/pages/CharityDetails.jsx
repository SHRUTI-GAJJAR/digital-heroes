...
function CharityDetails() {
  ...
  const loadCharity = async () => { setLoading(true); setError(""); setImageFailed(false); try { const { data } = await getCharity(id); setCharity(data.charity || null); } catch (requestError) { setError(apiMessage(requestError, "Unable to load this charity. Try again.")); } finally { setLoading(false); } };
