...
export default function Scores() {
  ...
  const confirmDelete = async () => { if (!deleteTarget) return; setDeleting(true); try { await deleteScore(deleteTarget.id); setSuccess("Score removed."); setDeleteTarget(null); await loadScores(); } catch (error) { setFormError(apiMessage(error, "We couldn’t remove this score.")); setDeleteTarget(null); } finally { setDeleting(false); } };
  return <section className="member-page"><header className="member-header"><div><p className="page-eyebrow">Golf activity</p><h1>Your Stableford scores.</h1><p>Keep your latest five scores up to date for your Digital Heroes subscription.</p></div></header>
    ...
    <div className="member-card form-card"><div className="card-heading"><h2>Recent scores</h2><span className="status-badge">{scores.length} {scores.length === 1 ? "score" : "scores"}</span></div>{loading ? <InlineLoading label="Loading your scores…" /> : loadError ? <ErrorState message={loadError} onRetry={loadScores} /> : scores.length === 0 ? <EmptyState title="No scores added yet." message="Add your latest Stableford score to keep your playing record up to date." /> : <div className="scores-table"><table><thead><tr><th>Date</th><th>Stableford score</th><th aria-label="Actions" /></tr></thead><tbody>{scores.map((score) => <tr key={score.id}><td>{formatDate(score.score_date)}</td><td className="score-cell">{score.stableford_score}</td><td><div className="row-actions"><button className="text-action" onClick={() => startEdit(score)}><Pencil size={15} /> Edit score</button><button className="text-action danger-action" onClick={() => setDeleteTarget(score)}><Trash2 size={15} /> Remove score</button></div></td></tr>)}</tbody></table></div>}</div>
